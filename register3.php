...
$register_email =        (!empty($_POST['register_email'])) ?           $_POST['register_email'] : '';  // disabled inputs do not submit
...
$register_league_id =    (!empty($_SESSION['registerconfirmleagueid'])) ?  $_SESSION['registerconfirmleagueid'] : '';
...
if (!$register_league_id || !$confirm_code) {
...
$msg = '';
while(1) {      
   if (empty($register_user_name) || empty($register_first_name) || empty($register_last_name) || empty($register_confirm_email) || empty($register_pw01) || empty($register_pw02)) {
      writeDataToFile("register3() $register_user_name || !$register_first_name || !$register_last_name || !$register_confirm_email || !$register_pw01 || !$register_pw02", __FILE__, __LINE__);
   ...
   if (!valid_email($register_confirm_email)) {
      $sub_error = true;
   ...
   if (!isUniqueEmailAddress($register_confirm_email)) {
      $sub_error = true;
...
if(!insertNewUser($register_user_name, $hash_password, $register_first_name, $register_last_name,
      $register_confirm_email, 'user', 1, $ref_new_user_id, $ref_status_text))
{
   formatSessionMessage("The system failed to install the new user '$register_user_name'.  Please contact the site administrator.  (ref:inu2)", 'danger', $msg, 
      "r3-120 '$register_user_name', '$hash_password', '$register_first_name', '$register_last_name', '$register_confirm_email' '$ref_new_user_id', text: $ref_status_text");
   setSessionMessage($msg, 'error');
   writeDataToFile("league2.php - insertNewUser($register_user_name, $hash_password, $register_first_name, $register_last_name,
      $register_confirm_email, 'user', 1, $ref_new_user_id, $ref_status_text) Failed.\n  Session: " . 
