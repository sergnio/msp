...
function sayKOCohortStandingsTable(
   ...
   writeDataToFile(" 1 support_ko_cohort2.php
...
left join nspx_leagueplayer as y on y.leagueid = p.league_id and y.userid = (select id from users where username = p.user limit 1)
   ...
   while (1) {
      ...
      if ($week_begin > $week_end) {
         formatSessionMessage("The beginning week is after the end.", 'info', $msg, "skc2-114 '$week_begin'$week_end'");
      ...
      if (!$ans = runSql($sql_pending, '', 0, $ref_status_text)) {
         if ($ans === false) {
            formatSessionMessage("We are unable to display standings at this time.", 'info', $msg, "skc2-153 $ref_status_text");
            setSessionMessage($msg, 'error');
            break;
         }
         if ($ans === null) {
            formatSessionMessage("We are unable to display standings at this time.", 'info', $msg, "skc2-158 $ref_status_text");
            setSessionMessage($msg, 'error');
            break;
         }
      }
      ...
      if (!$ans = runSql($sql_players, array("i", $league_id), 0, $ref_status_text)) {
         ...
         if ($ans === null) {
            formatSessionMessage("We are unable to display standings.  There are no players active in this league.", 'info', $msg, "skc2-158 $ref_status_text");
            setSessionMessage($msg, 'error');
            break;
         }
      }
      ...
      if (!$ans = runSql($mysql, array("iii", $league_id, $week_begin, $calculated_end_week))) {
         if ($ans === false) {
            formatSessionMessage("We are unable to display standings at this time.", 'info', $msg, "skc2-177 $ref_status_text");
            setSessionMessage($msg, 'error');
            break;
         }
         if ($ans === null) {
            formatSessionMessage("We are unable to display standings.  There is no league activity.", 'info', $msg, "skc2-182 $ref_status_text");
            setSessionMessage($msg, 'error');
            break;
         }
      }
      ...
      if ($are_no_players) {
         formatSessionMessage("No members of this league have yet made a play.", 'info', $msg, "skc2-256");
         setSessionMessage($msg, 'error');
         break;
      }
   ...
   for ($w = $week_begin; $w <= $calculated_end_week && !$game_is_over; $w++) {
      ...
      if (!winnersWeekLastManStanding( $a_players,
                                       ...
                                       $ref_status_text                 ))
      {
          formatSessionMessage("winnersWeekLastManStanding() failed", 'danger', $msg, "skc2-323 $ref_status_text");
          setSessionMessage($msg, 'error');
          return;
      }
