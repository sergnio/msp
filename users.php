  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="pw-modal-title">Change Password</h4>
      </div>
      <div class="modal-body">
        <div class="form-group">
        	<label for="newPasswordTB">New Password</label>
        	<input type="text" class="form-control" id="newPasswordTB" placeholder="New-Password" required="required">
        	<input type="hidden" name="username" id="usernameTB" />
        	<input type="hidden" name="userid" id="useridTB" />
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
        <button type="button" class="btn btn-primary" onClick="resetUser();">Email new Password</button>
      </div>
    </div><!-- /.modal-content -->
  </div><!-- /.modal-dialog -->
</div><!-- /.modal -->
...
<script>
	$(document).ready(function(){
		$("#passwordChangeModal").on("show.bs.modal", function(event){
			...
			var modal = $(this);
			modal.find("#pw-modal-title").text("Reset Password for " + username);
			modal.find("#usernameTB").val(username);
			modal.find("#useridTB").val(userid);				
		});
	});
	...
	function resetUser(e)
	{
		...
		var username = modal.find("#usernameTB").val();
		var password = modal.find("#newPasswordTB").val();
		var userid = modal.find("#useridTB").val();
		
		if(password=='')
		{
		 	modal.find("#newPasswordTB").closest('.form-group').addClass('has-error').addClass('has-feedback');
		}
		else
		{ //ok to submit
			//json data
			var data = { 'method': 'resetpassword', 'username' : username, 'password' : password, 'userid' : userid };
			
			$.ajax({ url: '/ajax_support_users.php',
